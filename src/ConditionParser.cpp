...
auto ConditionParser::BuildCondition(const std::vector<std::string>& a_conditionList) -> std::unique_ptr<RE::TESCondition>
{
	static srell::regex condRegex{ R"((\b\w+\b)(?:\s+(\b\w+\b))?(?:\s+(\b\w+\b))?\s*([<>]=?|==|!=)\s*(\d+\.\d+)\s*(AND|OR)?)" };

	auto tescondition = std::unique_ptr<RE::TESCondition>(nullptr);
	...
	for (auto& condition : a_conditionList) {
		...
		if (!tescondition) {
			tescondition = std::make_unique<RE::TESCondition>();
		}
